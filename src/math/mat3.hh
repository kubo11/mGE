...
struct mat3 {
 ...
 public:
  ...
  vec3<T>& operator[](int idx) { return col(idx); }

  const vec3<T>& operator[](int idx) const { return col(idx); }
...
template <typename T>
mat3<T> operator*(const T& a, const mat3<T>& m) {
  return mat3<T>(m[0] * a, m[1] * a, m[2] * a);
}

template <typename T>
vec3<T> operator*(const mat3<T>& m, const vec3<T>& v) {
  return vec3<T>(m.row(0) * v, m.row(1) * v, m.row(2) * v);
}

template <typename T>
mat3<T> operator*(const mat3<T>& m, const mat3<T>& n) {
  return mat3<T>(m * n[0], m * n[1], m * n[2]);
}
